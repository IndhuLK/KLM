  ...
  return (
    <div className="flex bg-gray-50">
      ...
      <main className="ml-5 flex-1 p-10">
        <h1 className="text-3xl font-bold text-[#EC3338] mb-8">Preview Blog</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {blogs.map((blog) => (
            <div key={blog.id} className="bg-white rounded-xl shadow-lg overflow-hidden transition-all hover:shadow-xl border border-gray-100">
              ...
              <div className="p-6">
                ...
                <p className="text-gray-500 text-sm mb-3">Posted on {blog.date}</p>
                <p className="text-gray-600 text-sm mb-3 line-clamp-3">{blog.description}</p>
                <div className="flex justify-between mt-5">
                  <button
                    onClick={() => navigate(`/edit-blog/${blog.id}`)}
                    className="flex items-center gap-2 text-sm font-semibold text-[#EC3338] hover:text-red-700 transition-all"
                  >
